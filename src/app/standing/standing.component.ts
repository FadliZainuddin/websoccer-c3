...
export class StandingComponent implements OnInit {
  ...
  public ComputeRankings(){
    var curData: Date = new Date();
    var TeamAt: number;
    this.Standings = [];
    this.MySchedule.forEach(element => {
      if(element.playingDate < curData && element.HomeScore>=0){
        TeamAt = this.FindTeam(element.HomeTeam);
        if(TeamAt < 0){
          this.Standings.push({
            TeamName:element.AwayTeam,
            GamePlayed:0,wins:0,Ties:0,GoalFor:0,GoalAgaints:0
          })
  ...
  private UpdCurrentRow(element:Schedule,TeamAt:number,HomeAway:string){
    ...
    if(HomeAway=="H"){
      this.Standings[TeamAt].GoalFor += element.HomeScore;
    ...
    if(HomeAway=="A"){
      this.Standings[TeamAt].GoalFor += element.AwayScore;
      this.Standings[TeamAt].GoalFor += element.HomeScore;
